...
function similar_posts_options_page() {
	...
	echo '</a></h2></div>';
	if ( ! SimilarPosts::check_post_plugin_library( '<h1>' . sprintf( __( 'Please install the %sPost Plugin Library%s plugin.' ), '<a href="http://downloads.wordpress.org/plugin/post-plugin-library.zip">', '</a>' ) . '</h1>' ) ) {
		return;
	}
...
function similar_posts_general_options_subpage() {
	...
	<div class="wrap">
		...
		<form method="post" action="">
			<div class="submit">
				<input type="submit" name="update_options" value="<?php _e( 'Save General Settings', 'similar_posts' ) ?>" />
			</div>
...
function similar_posts_output_options_subpage() {
	...
	<div class="wrap">
		...
		<form method="post" action="">
			<div class="submit">
				<input type="submit" name="update_options" value="<?php _e( 'Save Output Settings', 'similar_posts' ) ?>" />
			</div>
			<table class="optiontable form-table">
				<tr>
					<td>
...
function similar_posts_filter_options_subpage() {
	...
	<div class="wrap">
		...
		<form method="post" action="">
			<div class="submit">
				<input type="submit" name="update_options" value="<?php _e( 'Save Filter Settings', 'similar_posts' ) ?>" />
			</div>
			<table class="optiontable form-table">
				<?php
...
function similar_posts_placement_options_subpage() {
	...
	<div class="wrap">
		...
		<form method="post" action="">
			<div class="submit">
				<input type="submit" name="update_options" value="<?php _e( 'Save Placement Settings', 'similar_posts' ) ?>" />
			</div>
			<table class="optiontable form-table">
				<?php
...
function similar_posts_other_options_subpage() {
	...
	<div class="wrap">
		...
		<form method="post" action="">
			<div class="submit">
				<input type="submit" name="update_options" value="<?php _e( 'Save Other Settings', 'similar_posts' ) ?>" />
			</div>
			<table class="optiontable form-table">
				<?php
		...
		<form method="post" action="">
			<table class="optiontable form-table">
				<tr valign="top">
					...
					<td>
						<select name="utf8" id="utf8" <?php if ( ! function_exists( 'mb_split' ) ) {
							echo 'disabled="true"';
						} ?> >
							<option <?php if ( $options['utf8'] == 'false' ) {
								echo 'selected="selected"';
							} ?> value="false">No
							</option>
							<option <?php if ( $options['utf8'] == 'true' ) {
								echo 'selected="selected"';
							} ?> value="true">Yes
							</option>
						</select>
				...
				<tr valign="top">
					...
					<td>
						<select name="cjk" id="cjk" <?php if ( ! function_exists( 'mb_split' ) ) {
							echo 'disabled="true"';
						} ?> >
							<option <?php if ( $options['cjk'] == 'false' ) {
								echo 'selected="selected"';
							} ?> value="false">No
							</option>
							<option <?php if ( $options['cjk'] == 'true' ) {
								echo 'selected="selected"';
							} ?> value="true">Yes
							</option>
						</select>
				...
				<tr valign="top">
					...
					<td>
						<select name="use_stemmer" id="use_stemmer">
							<option <?php if ( $options['use_stemmer'] == 'false' ) {
								echo 'selected="selected"';
							} ?> value="false">Strictly
							</option>
							<option <?php if ( $options['use_stemmer'] == 'true' ) {
								echo 'selected="selected"';
							} ?> value="true">By Stem
							</option>
							<option <?php if ( $options['use_stemmer'] == 'fuzzy' ) {
								echo 'selected="selected"';
							} ?> value="fuzzy">Fuzzily
							</option>
						</select>
				...
				<tr valign="top">
					...
					<td><input name="batch" type="text" id="batch" value="<?php echo $options['batch']; ?>" size="3" />
					</td>
				</tr>
			</table>
			<div class="submit">
...
function similar_posts_install() {
	...
	if ( ! isset( $options['omit_current_post'] ) ) {
		$options['omit_current_post'] = 'true';
	}
	if ( ! isset( $options['show_private'] ) ) {
		$options['show_private'] = 'false';
	}
	if ( ! isset( $options['show_pages'] ) ) {
		$options['show_pages'] = 'false';
	}
	if ( ! isset( $options['show_attachments'] ) ) {
	...
	if ( ! isset( $options['none_text'] ) ) {
		$options['none_text'] = __( 'None Found', 'similar_posts' );
	}
	if ( ! isset( $options['no_text'] ) ) {
		$options['no_text'] = 'false';
	}
	if ( ! isset( $options['tag_str'] ) ) {
		$options['tag_str'] = '';
	}
	if ( ! isset( $options['excluded_cats'] ) ) {
		$options['excluded_cats'] = '';
	}
	if ( $options['excluded_cats'] === '9999' ) {
		$options['excluded_cats'] = '';
	}
	if ( ! isset( $options['included_cats'] ) ) {
		$options['included_cats'] = '';
	}
	if ( $options['included_cats'] === '9999' ) {
		$options['included_cats'] = '';
	}
	if ( ! isset( $options['excluded_authors'] ) ) {
		$options['excluded_authors'] = '';
	}
	if ( $options['excluded_authors'] === '9999' ) {
		$options['excluded_authors'] = '';
	}
	if ( ! isset( $options['included_authors'] ) ) {
		$options['included_authors'] = '';
	}
	if ( $options['included_authors'] === '9999' ) {
		$options['included_authors'] = '';
	}
	if ( ! isset( $options['included_posts'] ) ) {
		$options['included_posts'] = '';
	}
	if ( ! isset( $options['excluded_posts'] ) ) {
		$options['excluded_posts'] = '';
	}
	if ( $options['excluded_posts'] === '9999' ) {
		$options['excluded_posts'] = '';
	}
	if ( ! isset( $options['stripcodes'] ) ) {
		$options['stripcodes'] = array( array() );
	}
	if ( ! isset( $options['prefix'] ) ) {
		$options['prefix'] = 'Similar Posts:<ul>';
	}
	if ( ! isset( $options['suffix'] ) ) {
		$options['suffix'] = '</ul>';
	}
	if ( ! isset( $options['output_template'] ) ) {
		$options['output_template'] = '<li>{link}</li>';
	}
	if ( ! isset( $options['match_cat'] ) ) {
		$options['match_cat'] = 'false';
	}
	if ( ! isset( $options['match_tags'] ) ) {
		$options['match_tags'] = 'false';
	}
	if ( ! isset( $options['match_author'] ) ) {
		$options['match_author'] = 'false';
	}
	...
	if ( ! isset( $options['weight_content'] ) ) {
		$options['weight_content'] = 0.9;
	}
	if ( ! isset( $options['weight_title'] ) ) {
		$options['weight_title'] = 0.1;
	}
	if ( ! isset( $options['weight_tags'] ) ) {
		$options['weight_tags'] = 0.0;
	}
	if ( ! isset( $options['num_terms'] ) ) {
		$options['num_terms'] = 20;
	}
	if ( ! isset( $options['term_extraction'] ) ) {
		$options['term_extraction'] = 'frequency';
	}
	if ( ! isset( $options['hand_links'] ) ) {
		$options['hand_links'] = 'false';
	}
	update_option( 'similar-posts-feed', $options );
	...
	// check each of the option values and, if empty, assign a default (doing it this long way
	// lets us add new options in later versions)
	if ( ! isset( $options['feed_active'] ) ) {
		$options['feed_active'] = 'false';
	} // deprecated
	if ( ! isset( $options['widget_condition'] ) ) {
		$options['widget_condition'] = '';
	}
	if ( ! isset( $options['widget_parameters'] ) ) {
		$options['widget_parameters'] = '';
	}
	if ( ! isset( $options['feed_on'] ) ) {
		$options['feed_on'] = 'false';
	}
	if ( ! isset( $options['feed_priority'] ) ) {
		$options['feed_priority'] = '10';
	}
	if ( ! isset( $options['feed_parameters'] ) ) {
		$options['feed_parameters'] = 'prefix=<strong>' . __( 'Similar Posts', 'similar-posts' ) . ':</strong><ul class="similar-posts">&suffix=</ul>';
	}
	if ( ! isset( $options['append_on'] ) ) {
		$options['append_on'] = 'false';
	}
	if ( ! isset( $options['append_priority'] ) ) {
		$options['append_priority'] = '10';
	}
	if ( ! isset( $options['append_parameters'] ) ) {
		$options['append_parameters'] = 'prefix=<h3>' . __( 'Similar Posts', 'similar-posts' ) . ':</h3><ul class="similar-posts">&suffix=</ul>';
	}
	if ( ! isset( $options['append_condition'] ) ) {
		$options['append_condition'] = 'is_single()';
	}
	if ( ! isset( $options['limit'] ) ) {
		$options['limit'] = 5;
	}
	...
	if ( ! isset( $options['content_filter'] ) ) {
		$options['content_filter'] = 'false';
	}
	if ( ! isset( $options['custom'] ) ) {
		$options['custom']['key']   = '';
		$options['custom']['op']    = '=';
