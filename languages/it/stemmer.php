...
class ItalianStemmer {
	...
	function step_0( $str ) {
		...
		foreach ( $this->ante_suff_a as $ante_a ) {
			if ( $ante_suff_len - strlen( $ante_a ) < 0 ) {
				continue;
			}
		...
		foreach ( $this->ante_suff_b as $ante_b ) {
			if ( $ante_suff_len - strlen( $ante_b ) < 0 ) {
				continue;
			}
	...
	function delete_suff( $arr_suff, $str, $str_len, $where, $ovunque = false ) {
		if ( $where === 'r2' ) {
			$r = $this->return_R2( $str );
		} else if ( $where === 'rv' ) {
			$r = $this->return_RV( $str );
		} else if ( $where === 'r1' ) {
			$r = $this->return_R1( $str );
		}
		...
		if ( $ovunque ) {
			...
			foreach ( $arr_suff as $suff ) {
				if ( $r_len - strlen( $suff ) < 0 ) {
					continue;
				}
				$pos = strpos( $r, $suff, $r_len - strlen( $suff ) );
				if ( $pos !== false ) {
					return substr( $str, 0, $pos + $str_len - $r_len );
				}
			}
		}
	}
	...
	function step_1( $str ) {
		...
		if ( count( $ret_str = $this->delete_suff( $this->suffissi_step_1_g, $str, $str_len, 'r1' ) ) ) {
			if ( count( $ret_str1 = $this->delete_suff( array( 'iv' ), $ret_str, strlen( $ret_str ), 'r2' ) ) ) {
				if ( count( $ret_str2 = $this->delete_suff( array( 'at' ), $ret_str1, strlen( $ret_str1 ), 'r2' ) ) ) {
					return $ret_str2;
				} else {
					return $ret_str1;
				}
		...
		//delete if in R2
		if ( count( $ret_str = $this->delete_suff( $this->suffissi_step_1_a, $str, $str_len, 'r2', true ) ) ) {
			return $ret_str;
		}
		...
		if ( count( $ret_str = $this->delete_suff( $this->suffissi_step_1_c, $str, $str_len, 'r2' ) ) ) {
			return $ret_str . 'log';
		}
		//replace with 'u' if in R2 
		if ( count( $ret_str = $this->delete_suff( $this->suffissi_step_1_d, $str, $str_len, 'r2' ) ) ) {
			return $ret_str . 'u';
		}
		//replace with 'ente' if in R2 
		if ( count( $ret_str = $this->delete_suff( $this->suffissi_step_1_e, $str, $str_len, 'r2' ) ) ) {
			return $ret_str . 'ente';
		}
		//delete if in RV
		if ( count( $ret_str = $this->delete_suff( $this->suffissi_step_1_f, $str, $str_len, 'rv' ) ) ) {
			return $ret_str;
		}
		...
		if ( count( $ret_str = $this->delete_suff( $this->suffissi_step_1_i, $str, $str_len, 'r2' ) ) ) {
			if ( count( $ret_str1 = $this->delete_suff( array( 'at' ), $ret_str, strlen( $ret_str ), 'r2' ) ) ) {
				if ( count( $ret_str2 = $this->delete_suff( array( 'ic' ), $ret_str1, strlen( $ret_str1 ), 'r2' ) ) ) {
					return $ret_str2;
				} else {
					return $ret_str1;
				}
	...
	function step_2( $str, $str_step_1 ) {
		...
		if ( count( $ret_str = $this->delete_suff( $this->suffissi_step_2, $str, $str_len, 'rv' ) ) ) {
			return $ret_str;
		}
