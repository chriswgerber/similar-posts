...
$similar_posts_version = $similar_posts_feed_version = '1.0.0';
...
if ( ! defined( 'POST_PLUGIN_LIBRARY' ) ) {
	SimilarPosts::install_post_plugin_library();
}
	...
	function execute( $args = '', $default_output_template = '<li>{link}</li>', $option_key = 'similar-posts' ) {
		...
		if ( 0 < $options['limit'] ) {
			...
			if ( $omit_current_post ) {
				$where[] = where_omit_post( $similar_posts_current_ID );
			}
			if ( $hide_pass ) {
		...
		if ( $results ) {
			...
			if ( $options['sort']['by1'] !== '' ) {
				$items = ppl_sort_items( $options['sort'], $results, $option_key, $options['group_template'], $items );
			}
...
function widget_rrm_similar_posts_init() {
	...
	function widget_rrm_similar_posts_control() {
		...
		?>
		<p><label for="widget_rrm_similar_posts_title"> <?php _e( 'Title:', 'similar_posts' ); ?>
				<input style="width: 200px;" id="widget_rrm_similar_posts_title" name="widget_rrm_similar_posts_title" type="text" value="<?php echo $title; ?>" /></label>
		</p>
		<p><label for="widget_rrm_similar_posts_number"> <?php _e( 'Number of posts to show:', 'similar_posts' ); ?>
				<input style="width: 25px; text-align: center;" id="widget_rrm_similar_posts_number" name="widget_rrm_similar_posts_number" type="text" value="<?php echo $number; ?>" /></label> <?php _e( '(at most 15)', 'similar_posts' ); ?>
		</p>
		<p>
			<label for="widget_rrm_similar_posts_condition"> <?php echo sprintf( __( 'Show only if page: (e.g., %sis_single()%s)', 'similar_posts' ), '<a href="http://codex.wordpress.org/Conditional_Tags" title="help">', '</a>' ); ?>
				<input style="width: 200px;" id="widget_rrm_similar_posts_condition" name="widget_rrm_similar_posts_condition" type="text" value="<?php echo $condition; ?>" /></label>
		</p>
	...
	*/
	wp_register_sidebar_widget(
		'similar_posts_revised',
		__( 'Similar Posts +', 'similar_posts' ),
...
function similar_posts_init() {
	...
	if ( $options['feed_on'] === 'true' && function_exists( 'ppl_register_post_filter' ) ) {
		ppl_register_post_filter( 'feed', 'similar-posts', 'SimilarPosts' );
	}
	...
	if ( $options['append_on'] === 'true' && function_exists( 'ppl_register_post_filter' ) ) {
		ppl_register_post_filter( 'append', 'similar-posts', 'SimilarPosts', $condition );
	}
